...
st.caption("Search stocks by ticker or name, compare valuation, review analyst targets, inspect leadership, and view your Trading 212 portfolio.")
...
t212_api_secret = st.sidebar.text_input("Trading 212 API Secret", type="password")
...
if "analysis_nonce" not in st.session_state:
    st.session_state["analysis_nonce"] = 0
if "portfolio_overrides" not in st.session_state:
    st.session_state["portfolio_overrides"] = {}
...
def candidate_yf_tickers(t212_ticker):
    if not t212_ticker:
        return []
    t = str(t212_ticker).strip().upper()
    parts = t.split("_")
    symbol = clean_symbol(parts[0])
    exchange = parts[1] if len(parts) >= 2 else ""
    suffix_map = {
        "UK": ".L", "GB": ".L", "DE": ".DE", "XETRA": ".DE", "FR": ".PA", "PAR": ".PA",
        "NL": ".AS", "AMS": ".AS", "ES": ".MC", "IT": ".MI", "SE": ".ST", "DK": ".CO",
        "NO": ".OL", "FI": ".HE", "CH": ".SW", "AU": ".AX", "CA": ".TO", "HK": ".HK",
        "JP": ".T", "US": "", "NYSE": "", "NASDAQ": ""
    }
    candidates = []
    if symbol:
        candidates.append(symbol)
    if symbol and exchange in suffix_map:
        mapped = symbol + suffix_map[exchange]
        if mapped not in candidates:
            candidates.append(mapped)
    if symbol:
        for suf in [".L", ".TO", ".AX", ".PA", ".AS", ".DE", ".SW", ".MI", ".MC", ".ST", ".OL", ".HE", ".HK", ".T"]:
            cand = symbol + suf
...
def yahoo_search(query, max_results=8):
    try:
...
def score_candidate(candidate, base_symbol, instrument_name):
    score = 0.0
    symbol = (candidate.get("symbol") or "").upper()
    name = (candidate.get("name") or "").upper()
    base_symbol = (base_symbol or "").upper()
    instrument_name = (instrument_name or "").upper()
    if symbol == base_symbol:
        score += 100
    elif symbol.startswith(base_symbol):
        score += 45
    if instrument_name:
        score += 60 * SequenceMatcher(None, instrument_name, name).ratio()
    if candidate.get("type") in ["EQUITY", "ETF", "MUTUALFUND"]:
        score += 10
    return score

@st.cache_data(ttl=43200)
def resolve_yf_ticker(t212_ticker, instrument_name=""):
    base_candidates = candidate_yf_tickers(t212_ticker)
    base_symbol = clean_symbol(t212_ticker.split("_")[0] if t212_ticker else "")

    for cand in base_candidates:
        try:
            info = yf.Ticker(cand).info or {}
            if info.get("symbol") or info.get("currentPrice") or info.get("regularMarketPrice"):
                return cand
        except Exception:
            pass
    ...
    searches = []
    if instrument_name:
        searches.extend(yahoo_search(instrument_name, max_results=10))
    if base_symbol:
        searches.extend(yahoo_search(base_symbol, max_results=10))

    if searches:
        dedup = {}
        for r in searches:
            dedup[r["symbol"]] = r
        ranked = sorted(dedup.values(), key=lambda x: score_candidate(x, base_symbol, instrument_name), reverse=True)
        if ranked:
...
with main_tabs[0]:
    ...
    search_query = st.text_input("Predictive search", value="", placeholder="Try Apple, Tesla, NVIDIA, Microsoft...", key=f"predictive_search_{st.session_state['analysis_nonce']}")
    if search_query:
        results = yahoo_search(search_query, max_results=8)
        if results:
            options = [f"{r['symbol']} — {r['name']} ({r['exchange']})" for r in results]
            choice = st.radio("Best matches", options, key=f"predictive_choice_{st.session_state['analysis_nonce']}")
            if st.button("Load selected result", key=f"load_selected_result_{st.session_state['analysis_nonce']}"):
                idx = options.index(choice)
                st.session_state["jump_to_ticker"] = results[idx]["symbol"]
                st.session_state["analysis_nonce"] += 1
                st.rerun()
        else:
            st.info("No Yahoo Finance matches found for that search.")

    if st.session_state["jump_to_ticker"]:
        st.info(f"Showing analysis for **{active_ticker}**.")
        if st.button("← Back to manual search", key=f"back_manual_{st.session_state['analysis_nonce']}"):
            st.session_state["jump_to_ticker"] = None
            st.session_state["analysis_nonce"] += 1
            st.rerun()
    ...
    if ticker:
        try:
            ...
            with atabs[0]:
                ...
                with right:
                    ...
                    if not hist.empty:
                        ...
                        fig.update_layout(height=420, margin=dict(l=20, r=20, t=30, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#e6edf7"), xaxis=dict(gridcolor="rgba(255,255,255,0.05)"), yaxis=dict(gridcolor="rgba(255,255,255,0.05)"))
                        st.plotly_chart(fig, use_container_width=True, key=f"price_chart_{ticker}_{st.session_state['analysis_nonce']}")
                    else:
            ...
            with atabs[1]:
                ...
                val_df, missed = valuation_table(info, current, industry_pe, dcf_growth, dcf_margin, dcf_discount, dcf_terminal, dcf_years, cf)
                if not val_df.empty:
                    st.dataframe(val_df, use_container_width=True, hide_index=True, key=f"val_df_{ticker}_{st.session_state['analysis_nonce']}")
                    chart_df = val_df.copy().dropna(subset=["Implied ($)"])
                    if not chart_df.empty:
                        chart_df = chart_df.sort_values("Implied ($)")
                        chart_df["Color"] = chart_df["Upside %"].apply(lambda x: "#22c55e" if pd.notnull(x) and x >= 0 else "#f87171")
                        fig_val = go.Figure(go.Bar(
                            x=chart_df["Method"],
                            y=chart_df["Implied ($)"],
                            marker_color=chart_df["Color"],
                            text=[f"${v:,.2f}" for v in chart_df["Implied ($)"]],
                            textposition="outside",
                            hovertemplate="<b>%{x}</b><br>Implied Price: $%{y:,.2f}<extra></extra>"
                        ))
                        if current:
                            fig_val.add_hline(y=current, line_dash="dash", line_color="#38bdf8", annotation_text=f"Current ${current:,.2f}", annotation_position="right")
                        fig_val.update_layout(
                            title="Valuation Method Price Targets",
                            height=420,
                            ...
                            yaxis=dict(title="Implied Price ($)", gridcolor="rgba(255,255,255,0.05)")
                        )
                        st.plotly_chart(fig_val, use_container_width=True, key=f"valuation_chart_{ticker}_{st.session_state['analysis_nonce']}")
            ...
            with atabs[2]:
                ...
                if isinstance(fin, pd.DataFrame) and not fin.empty:
                    ...
                    rows_c.append({"Source": "Finnhub", "Low": r.get("targetLow"), "Median": r.get("targetMedian"), "Mean": r.get("targetMean"), "High": r.get("targetHigh"), "Last updated": r.get("lastUpdated", "")})
                rows_c.append({"Source": "Yahoo Finance", "Low": sn(info.get("targetLowPrice")), "Median": sn(info.get("targetMedianPrice")), "Mean": sn(info.get("targetMeanPrice")), "High": sn(info.get("targetHighPrice")), "Last updated": ""})
                st.dataframe(pd.DataFrame(rows_c), use_container_width=True, hide_index=True, key=f"targets_{ticker}_{st.session_state['analysis_nonce']}")
            with atabs[3]:
                ...
                if combined.empty:
                    ...
                    st.dataframe(combined[show_cols] if show_cols else combined, use_container_width=True, hide_index=True, key=f"history_{ticker}_{st.session_state['analysis_nonce']}")
            with atabs[4]:
                ...
                if ds is not None:
                    st.dataframe(ds if show_full else ds.head(), use_container_width=True, key=f"income_{ticker}_{st.session_state['analysis_nonce']}")
                else:
                    st.info("No income statement data available.")
                st.subheader("Quarterly balance sheet")
                if isinstance(qbs, pd.DataFrame) and not qbs.empty:
                    st.dataframe(qbs if show_full else qbs.head(), use_container_width=True, key=f"bs_{ticker}_{st.session_state['analysis_nonce']}")
                else:
                    st.info("No balance sheet data available.")
                st.subheader("Cash flow statement")
                if isinstance(cf, pd.DataFrame) and not cf.empty:
                    st.dataframe(cf if show_full else cf.head(), use_container_width=True, key=f"cf_{ticker}_{st.session_state['analysis_nonce']}")
                else:
...
with main_tabs[1]:
    if not t212_api_key or not t212_api_secret:
        ...
        if portfolio["error"]:
            ...
            s4.metric("Unrealised P&L", f"{currency} {unreal_pl:+,.2f}" if unreal_pl is not None else "NA", delta=f"{(unreal_pl / invested * 100):.1f}%" if unreal_pl and invested else None)
            ...
            if not positions:
                ...
                for p in positions:
                    ...
                    override_symbol = st.session_state["portfolio_overrides"].get(t212_tick)
                    yf_tick = override_symbol or resolve_yf_ticker(t212_tick, name)
                ...
                holdings_search = st.text_input("Search holdings by name or ticker", value="", placeholder="e.g. Apple, TSLA, VUSA", key=f"holdings_search_{st.session_state['analysis_nonce']}")
                if holdings_search:
                    q = holdings_search.lower().strip()
                    pos_df = pos_df[
                        pos_df["Name"].astype(str).str.lower().str.contains(q, na=False) |
                        pos_df["T212 Ticker"].astype(str).str.lower().str.contains(q, na=False) |
                        pos_df["YF Ticker"].astype(str).str.lower().str.contains(q, na=False)
                ...
                with ch1:
                    ...
                    if not pie_df.empty:
                        fig_pie = px.pie(pie_df, values="Market Value", names="Name", title="Allocation by Market Value", color_discrete_sequence=px.colors.sequential.Teal)
                        fig_pie.update_layout(paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#e6edf7"), legend=dict(font=dict(size=11)))
                        st.plotly_chart(fig_pie, use_container_width=True, key="portfolio_pie")
                with ch2:
                    pnl_df = pos_df.dropna(subset=["P&L"]).sort_values("P&L")
                    if not pnl_df.empty:
                        fig_bar = go.Figure(go.Bar(x=pnl_df["Name"], y=pnl_df["P&L"], marker_color=["#22c55e" if v >= 0 else "#f87171" for v in pnl_df["P&L"]], text=[f"{currency}{v:+,.2f}" for v in pnl_df["P&L"]], textposition="outside"))
                        fig_bar.update_layout(title="Unrealised P&L per holding", height=380, margin=dict(l=20, r=20, t=50, b=80), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#e6edf7"), yaxis=dict(gridcolor="rgba(255,255,255,0.05)"))
                        st.plotly_chart(fig_bar, use_container_width=True, key="portfolio_bar")
                st.divider()
                ...
                selected = st.dataframe(
                    ...
                    key=f"holdings_table_{st.session_state['analysis_nonce']}"
                )
                sel_rows = selected.selection.rows if hasattr(selected, "selection") else []
                if sel_rows:
                    sel_row = pos_df.iloc[sel_rows[0]]
                    sel_name = sel_row["Name"]
                    sel_t212 = sel_row["T212 Ticker"]
                    sel_yf = sel_row["YF Ticker"]
                    st.info(f"Selected: **{sel_name}** — current Yahoo Finance ticker: `{sel_yf}`")
                    manual_query = st.text_input("Fix this holding's Yahoo ticker by searching name or symbol", value=sel_name, key=f"manual_fix_query_{sel_t212}_{st.session_state['analysis_nonce']}")
                    manual_results = yahoo_search(manual_query, max_results=8) if manual_query else []
                    if manual_results:
                        manual_options = [f"{r['symbol']} — {r['name']} ({r['exchange']})" for r in manual_results]
                        manual_choice = st.selectbox("Possible matches", manual_options, key=f"manual_choice_{sel_t212}_{st.session_state['analysis_nonce']}")
                        cfix1, cfix2 = st.columns(2)
                        with cfix1:
                            if st.button("Save ticker override", key=f"save_override_{sel_t212}_{st.session_state['analysis_nonce']}"):
                                idx = manual_options.index(manual_choice)
                                chosen_symbol = manual_results[idx]["symbol"]
                                st.session_state["portfolio_overrides"][sel_t212] = chosen_symbol
                                st.session_state["analysis_nonce"] += 1
                                st.rerun()
                        with cfix2:
                            if st.button(f"📈 Analyse {sel_name} →", type="primary", key=f"analyse_btn_{sel_t212}_{st.session_state['analysis_nonce']}"):
                                final_symbol = st.session_state["portfolio_overrides"].get(sel_t212, sel_yf)
                                st.session_state["jump_to_ticker"] = final_symbol
                                st.session_state["analysis_nonce"] += 1
                                st.rerun()
                    else:
                        if st.button(f"📈 Analyse {sel_name} →", type="primary", key=f"analyse_btn_simple_{sel_t212}_{st.session_state['analysis_nonce']}"):
